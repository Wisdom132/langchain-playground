...
let _cerebras_cerebras_cloud_sdk = require("@cerebras/cerebras_cloud_sdk");
_cerebras_cerebras_cloud_sdk = require_runtime.__toESM(_cerebras_cerebras_cloud_sdk);
...
* <br />
*/
var ChatCerebras = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
		return "ChatCerebras";
	}
	lc_serializable = true;
	get lc_secrets() {
		return { apiKey: "CEREBRAS_API_KEY" };
	}
	get lc_aliases() {
		return { apiKey: "CEREBRAS_API_KEY" };
	}
	getLsParams(options) {
		...
		return {
			...
			ls_max_tokens: params.max_completion_tokens ?? void 0,
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields);
		this._addVersion("@langchain/cerebras", "1.0.4");
		...
		this.client = new _cerebras_cerebras_cloud_sdk.default({
			apiKey: fields.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("CEREBRAS_API_KEY"),
	...
	*/
	invocationParams(options) {
		return {
			...
			tool_choice: require_utils.formatToCerebrasToolChoice(options?.tool_choice)
		};
	}
	async _generate(messages, options, runManager) {
		...
		const { choices, usage, ...rest } = await this.caller.call(async () => {
			return await this.client.chat.completions.create({
				...this.invocationParams(options),
				messages: require_utils.convertToCerebrasMessageParams(messages),
				stream: false
			}, {
	...
	*/
		...
		for await (const chunk of stream) {
			...
			const generationChunk = new _langchain_core_outputs.ChatGenerationChunk({
				...
				message: new _langchain_core_messages.AIMessageChunk({
					content,
					tool_call_chunks: choice?.delta.tool_calls?.map((toolCallChunk) => ({
						id: toolCallChunk.id,
			...
			await runManager?.handleLLMNewToken(content, void 0, void 0, void 0, void 0, { chunk: generationChunk });
		}
	}
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonMode") throw new Error(`Cerebras withStructuredOutput implementation only supports "functionCalling" as a method.`);
		...
		return (0, _langchain_core_language_models_structured_output.assembleStructuredOutputPipeline)(this.bindTools(tools, { tool_choice: tools[0].function.name }), _langchain_core_runnables.RunnableLambda.from((input) => {
		...
		}), includeRaw, "ChatCerebrasStructuredOutput");
	}
};
